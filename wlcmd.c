...
/* commandTranslator
 * Translates your command based on your flag.
 * e.g. *nix -> Windows
 */
int commandTranslator(char * command, Flag flag)
{
    ...
    {
        // Linux Command
        cmdIndex = getIndex(command, nixCommands);

        if (cmdIndex >= 0)
        {
            printf("%s: %s", winCommands[cmdIndex], desCommands[cmdIndex]);
        }
        else
        {
    ...
    {
        // Windows command
        cmdIndex = getIndex(command, winCommands);

        if (cmdIndex != -1)
        {
            printf("%s", nixCommands[cmdIndex]);
        }
        else
        {
            return error(eCommand);
    ...
    else if (flag == fNixDes)
    {
        // Linux Command with description
        cmdIndex = getIndex(command, nixCommands);
    ...
    else if (flag == fWindowsDes)
    {
        // Windows command with description
        cmdIndex = getIndex(command, winCommands);

        if (cmdIndex != -1)
        {
            printf("%s : %s", nixCommands[cmdIndex], desCommands[cmdIndex]);
        }
        else
        {
            return error(eCommand);
